...
static InputStream* inStream = 0;
//static void _png_read(png_structp t, png_bytep data, png_uint_32 size) {
#ifdef MAC
static void _png_read(png_structp t, png_bytep data, png_size_t size) {
	inStream->getBlock((char*)data, size);
#else
static void _png_read(png_structp t, png_bytep data, unsigned int size) {
	inStream->getBlock((char*)data, size);

#endif
}

Image* Image::createFromPNG(InputStream* in) {
	png_byte header[8];
	...
	inStream = in;
	png_set_read_fn(readStruct, NULL, _png_read);
