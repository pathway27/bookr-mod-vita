...
FZFont* FZFont::createUTFFromFile(char* fileName, int fontSize, bool autohint) {
	// This initializes FreeType
	FT_Library library;
	if (FT_Init_FreeType(&library) != 0) {
	  //	printf("cannot init freetype\n");
	...
	FZFont* ret = createProto(library, face, fontSize, autohint);
	ret->isUTF = true;
...
FZFont* FZFont::createUTFFromMemory(unsigned char* buffer, int bufferSize, int fontSize, bool autohint) {
	// This initializes FreeType
	FT_Library library;
	if (FT_Init_FreeType(&library) != 0) {
	  //printf("cannot init freetype\n");
...
int FZFont::getSingleMetrics(unsigned long idx, FZCharMetrics* met, FZTexture** fztexturepp){
  int offset = 0;
  ...
  if(!initUTFFont())
    ...
    // Allocate memory for the texture
    int clutSize = 256;
    FZImage* image = FZImage::createEmpty(image_width, image_height, clutSize, FZImage::mono8);
    unsigned char* fontTexture = (unsigned char*)image->getData();
    // add clut to texture
    unsigned int* clut = image->getCLUT();
    ...
    *fztexturepp = new FZFont();

    if( *fztexturepp == 0 || !initFromImage(*fztexturepp,image,false)){
      if (*fztexturepp){
	((FZFont*)(*fztexturepp)) -> release();
	*fztexturepp = 0;
      }
      image->release();
      return 0;
       ...
       ((FZFont*)(*fztexturepp))->metrics = 0;
       ((FZFont*)(*fztexturepp))->metrics = (FZCharMetrics*)memalign(16,sizeof(FZCharMetrics));
       if (((FZFont*)(*fztexturepp))->metrics){
 	memset(((FZFont*)(*fztexturepp))->metrics, 0, sizeof(FZCharMetrics));
 	((FZFont*)(*fztexturepp))->metrics->width = met->width;
 	((FZFont*)(*fztexturepp))->metrics->height = met->height;
 	((FZFont*)(*fztexturepp))->metrics->xoffset = met->xoffset;
 	((FZFont*)(*fztexturepp))->metrics->yoffset = met->yoffset;
 	((FZFont*)(*fztexturepp))->metrics->x = met->x;
 	((FZFont*)(*fztexturepp))->metrics->y = met->y;
 	((FZFont*)(*fztexturepp))->metrics->xadvance = met->xadvance;
       }
       ((FZFont*)(*fztexturepp))->lineHeight = this->lineHeight;
       ((FZFont*)(*fztexturepp))->maxAscent = this->maxAscent;
      (*fztexturepp)->texEnv(FZ_TEX_MODULATE);
      (*fztexturepp)->filter(FZ_NEAREST, FZ_NEAREST);
      image->release();
      return 1;
    }
}

int FZFont::get_next_utf8_char(unsigned long* ucs, const char* s, int n) {
	const char *src = s;
	char c;
	int extra;
...
FZTexture* FZFont::getTextureFromString(const char* t,int cgap){
  int margin=3;
  ...
  struct FZCharMetrics* fontMetrics = (FZCharMetrics*)memalign(16, 1 * sizeof(FZCharMetrics));
  if(!fontMetrics)
    return 0;
  memset(fontMetrics, 0, 1 * sizeof(FZCharMetrics));
