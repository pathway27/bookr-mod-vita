...
static struct BKCharMetrics fontMetrics[256];
...
int bkFontPrecalc(char* fileName, int font_size) {
	...
	fontTexture = (unsigned char*)memalign(16, image_height * image_width);
	for (int i = 0; i < image_height * image_width; ++i)
		fontTexture[i] = 0;
	fontTextureH = image_height;
	fontLineHeight = max_ascent + max_descent + margin;
...
int bkFontGetTextureHeight() {
	return fontTextureH;
}

int bkFontLineHeight() {
	return fontLineHeight;
}

unsigned char* bkFontGetTexture() {
	return fontTexture;
}

BKCharMetrics* bkFontGetMetrics() {
	return fontMetrics;
}
